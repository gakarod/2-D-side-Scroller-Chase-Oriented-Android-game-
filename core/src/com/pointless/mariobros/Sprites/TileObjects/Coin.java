package com.pointless.mariobros.Sprites.TileObjects;
        ...
        if(getCell().getTile().getId() == BLANK_COIN)
            ...
            if(object.getProperties().containsKey("mushroom")) {
                screen.spawnItem(new ItemDef(new Vector2(body.getPosition().x, body.getPosition().y + 16 / MarioBros.PPM),
                        com.pointless.mariobros.Sprites.Items.Mushroom.class));
                MarioBros.manager.get("audio/sounds/powerup_spawn.wav", Sound.class).play();
            }
            else
                MarioBros.manager.get("audio/sounds/coin.wav", Sound.class).play();
            getCell().setTile(tileSet.getTile(BLANK_COIN));
            com.pointless.mariobros.Scenes.Hud.addScore(100);
        }
    }
}
